...
namespace FreedomVoice.iOS.Utilities.Helpers
{
    public static class Log
    {
        ...
        public static void ReportTime(EventCategory eventCategory, string name, string result, long time)
        {
            ...
            GAI.SharedInstance.DefaultTracker.Send(GAIDictionaryBuilder.CreateTiming(category, time, name, result).Build());
        }

        public static void ReportEvent(string name, string result)
        {
            GAI.SharedInstance.DefaultTracker.Send(GAIDictionaryBuilder.CreateEvent(OtherKey, name, result, 1).Build());
        }
    }
}